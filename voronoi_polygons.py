...
import polymap.shapely.geometry as po
    ...
    for i, triangle in enumerate(triangles):
        ...
        for j, neighbor in enumerate(delauny.neighbors[i]):
            if neighbor != -1:
                ...
                lineIndices.append(
                    (i, len(circum_centers) + len(long_lines_endpoints) - 1))
...
def voronoi_polygons(cells):
    ...
    for pIdx, lineIndices_ in cells.items():
        ...
        for i1, i2 in lineIndices_:
            connections = list(
                filter(lambda i12_: (i1, i2) != (i12_[0], i12_[1]) and
                                    (i1 == i12_[0] or i1 == i12_[1] or i2 ==
                                     i12_[0] or i2 == i12_[1]),
                       lineIndices_))
        ...
        if len(dangling_lines) == 2:
            ...
            connected = list(filter(
                lambda i12_: (i12_[0], i12_[1]) != (i11, i12) and (
                i12_[0] == i11 or i12_[1] == i11), lineIndices_))
            i11Unconnected = len(connected) == 0

            connected = list(filter(
                lambda i12_: (i12_[0], i12_[1]) != (i21, i22) and (
                i12_[0] == i21 or i12_[1] == i21), lineIndices_))
    ...
    for pIdx, lineIndices_ in cells.items():
        ...
        while len(orderedEdges) < len(lineIndices_):
            i1 = currentEdge[1]
            i2 = directedGraphMap[i1][0] if directedGraphMap[i1][0] != \
                                            currentEdge[0] else \
            directedGraphMap[i1][1]
            nextEdge = (i1, i2)
...
def polygons(points):
    ...
    for i in range(len(points)):
        poly_points = vertices[np.asarray(polys[i])]
        # poly_line_indices = lineIndices[np.asarray(polys[i])]

        # print(poly_points)
        # # print(poly_line_indices)
        # ll = np.array([0,0])
        #
        # ur = np.array([1,1])
        #
        # # find vertex is inside of boundries and outside of boundries
        # indix = np.all( np.logical_and(ll <= poly_points, poly_points <=ur), axis=1)
        # inbox =poly_points[indix]
        # outbox =poly_points[np.logical_not(indix)]
        #
        # subtract_area = []
        #
        # for i in range(poly_points.size):
        #     if poly_points[i] in outbox:
        #
        #         if not (poly_points[i-1] in inbox or poly_points[i+1] in inbox):
        #             print("error")
        #
        #
        #
...
if __name__ == '__main__':
    ...
    for poly in polys:
        p = matplotlib.patches.Polygon(poly,
